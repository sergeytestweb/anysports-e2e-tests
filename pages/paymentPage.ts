...
export function createPaymentPage(page: Page) {
  ...
  const cardNumberInput: Locator = page.getByRole('textbox', { name: 'Номер карты' });
  const expiryDateInput: Locator = page.getByRole('textbox', { name: 'MM/ГГ' });
  const cvvInput: Locator = page.getByRole('textbox', { name: 'CVV' });
  
  // Другие локаторы
  const submitPaymentButton: Locator = page.getByRole('button', { name: 'Оплатить 11.35₽' });
  const errorMessage: Locator = page.getByText('Недостаточно средств на карте');
  const loadingIndicator: Locator = page.getByRole('heading', { name: 'Подождите, идет обработка платежа' });
